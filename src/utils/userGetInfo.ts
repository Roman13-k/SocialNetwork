...
export function getUserAvatar(user: CurrentProfileType): string {
  if (!user) return "";
  return "user_metadata" in user ? user.user_metadata?.avatar_url ?? "" : user.avatar_url ?? "";
}

export function getUserEmail(user: CurrentProfileType): string | undefined {
  if (!user) return undefined;
  return "user_metadata" in user ? user.user_metadata?.email : "not found";
}
